...
test.describe('Logout User Flow', () => {
  ...
  test('should register, login and logout user successfully', async ({ page }) => {
    await test.step('Launch browser and navigate to home page', async () => {
      await homePage.goto();
      await expect(page).toHaveURL(URL_PATTERNS.HOME);
    });

    await test.step('Verify home page is visible (logo, nav, banners)', async () => {
    ...
    await test.step('Verify redirect to Login page and login form is visible', async () => {
